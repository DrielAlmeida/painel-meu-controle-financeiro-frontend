...
export default function LoginPage() {
  ...
  const { entrar: autenticar, carregando: authLoading } = useAuth();
  ...
  async function entrar(event: React.FormEvent<HTMLFormElement>) {
    ...
    setCarregando(true);
    try {
      await autenticar({ telefone, senha });
  ...
  return (
    <AuthCard title="Entrar" subtitle="Acesse usando seu telefone e senha." footer={<>Ainda não possui senha? <Link className="font-semibold text-blue-600" href="/checkout">Criar acesso</Link></>}>
      <form className="grid gap-4" onSubmit={entrar}>
        ...
        <Button type="submit" size="lg" disabled={carregando || authLoading}>{carregando ? "Entrando..." : "Entrar"}</Button>
      </form>
    </AuthCard>
  );
}

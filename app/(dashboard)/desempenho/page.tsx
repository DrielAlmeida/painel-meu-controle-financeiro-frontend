...
export default function DesempenhoPage() {
  ...
  const analysis = useMemo(
    () =>
      summarizeIncidents(
        gastos.filter((gasto) => !gasto.excluido),
  ...
  const computedLevel =
    data.percentual_utilizado > 100
      ? "critical"
      : data.percentual_utilizado >= 80 ||
          analysis.attentionPurchases.length > 0 ||
          analysis.incidents.length >= 5
        ? "attention"
    ...
    <div className="space-y-6">
      <div>
        <p className="text-sm font-semibold text-blue-500">
          Central de segurança financeira
        </p>
        <h1 className="text-3xl font-black">Desempenho</h1>
        <p className="mt-1 text-sm text-slate-500">
          A análise separa compras normais de incidentes com impacto real no
          orçamento.
        </p>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <Card>
          <div className="flex items-center justify-between">
            <span className="text-sm text-slate-500">Sequência segura</span>
            <CalendarCheck2 className="text-emerald-500" />
          </div>
          <strong className="mt-3 block text-3xl">
            {data.dias_sem_incidentes} dias
          </strong>
          <p className="mt-1 text-xs text-slate-500">
            Desde o último impacto relevante
          </p>
        </Card>
        <Card>
          <div className="flex items-center justify-between">
            <span className="text-sm text-slate-500">Incidentes reais</span>
          ...
          <p className="mt-1 text-xs text-slate-500">
            Compras fora do planejamento com impacto
          </p>
        </Card>
        <Card>
          <div className="flex items-center justify-between">
            <span className="text-sm text-slate-500">Compras normais</span>
            <CheckCircle2 className="text-blue-400" />
          </div>
          <strong className="mt-3 block text-3xl">
            {analysis.normalPurchases.length}
          </strong>
          <p className="mt-1 text-xs text-slate-500">
            Essenciais, planejadas ou recorrentes
          </p>
        </Card>
        <Card>
          <div className="flex items-center justify-between">
            <span className="text-sm text-slate-500">Risco atual</span>
          ...
          <p className="mt-1 text-xs text-slate-500">
            {data.percentual_utilizado.toFixed(1)}% da meta utilizada
          </p>
        </Card>
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.1fr_.9fr]">
        <Card>
          ...
          <div className="mt-4 space-y-3">
            {analysis.classified
              .slice(0, 8)
              .map(({ gasto, classification, reason }) => (
                <div
                  key={gasto.id}
        ...
        <Card>
          ...
          <div className="mt-5 space-y-3">
            {[
              {
                name: "Compra normal",
                range: "Planejada, recorrente ou essencial de baixo valor",
                color: "bg-emerald-500",
              },
              {
                name: "Incidente leve",
                range: "Compra avulsa pequena fora do planejamento",
                color: "bg-amber-400",
              },
              {
                name: "Em observação",
                range: "80% da meta ou compra de impacto relevante",
                color: "bg-orange-500",
              },
              {
                name: "Estado crítico",
                range: "Acima de 100% da meta mensal",
                color: "bg-red-500",
              },
            ].map((item) => (
              <div
          ...
          <p className="mt-5 rounded-xl bg-blue-50 p-3 text-xs text-blue-700 dark:bg-blue-950/40 dark:text-blue-300">
            Exemplo: pão de R$ 5,00 em alimentação é considerado uma compra
            cotidiana, não um incidente.
          </p>
        </Card>
      </div>
    </div>
